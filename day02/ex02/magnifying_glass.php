...
if ($argc > 0)
{
	...
	$match = preg_replace_callback("/.*<a href=.+>(.+)<\/a>/mi", "replace", $line);
	$match = preg_replace_callback("/.*<a href=.+>(.+)/mi", "replace", $match);
	$match = preg_replace_callback("/.*<a href=.+>(.+)<.*><\/a>/mi", "replace", $match);
